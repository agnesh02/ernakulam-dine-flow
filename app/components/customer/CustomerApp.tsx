...
export const CustomerApp = () => {
  ...
  const handleOrderPlaced = (orderOrGroupId: string | { id: string }) => {
    ...
    if (typeof orderOrGroupId === 'string') {
      ...
      // Keep existing orderGroupId if we already have one (for continuous ordering)
      if (!orderGroupId) {
        setOrderGroupId(orderOrGroupId);
        localStorage.setItem('currentOrderGroupId', orderOrGroupId);
      }
      // Don't clear single order tracking - keep both for flexibility
    } else {
      // Single restaurant order - store order object
      // If we already have an orderGroupId, keep it (the backend creates orders within the group)
      if (!orderGroupId) {
        setOrderId(orderOrGroupId.id);
        setCurrentOrder(orderOrGroupId as any);
        localStorage.setItem('currentOrderId', orderOrGroupId.id);
      }
      // Extract and store orderGroupId if present in the response
      if ((orderOrGroupId as any).orderGroupId) {
        const groupId = (orderOrGroupId as any).orderGroupId;
        setOrderGroupId(groupId);
        localStorage.setItem('currentOrderGroupId', groupId);
      }
    }
  ...
  const handleStartNewOrder = () => {
    ...
    toast({
      ...
      description: "Select a restaurant to continue ordering",
    });
  };
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="menu">
          <DigitalMenu 
            ...
            existingOrderGroupId={orderGroupId}
          />
        </TabsContent>
